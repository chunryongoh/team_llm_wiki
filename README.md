...
Daily brief generation writes `wiki/briefs/<date>-daily.md` and keeps `wiki/briefs/latest.md` as a pointer to the newest daily brief. Weekly brief generation writes `wiki/briefs/<YYYY-Www>-weekly.md` with a contradiction scan and `wiki/briefs/<date>-stale-claims.md` for tentative claims older than 14 days. Generated brief pages are excluded from wiki health link, orphan, claim, and metric checks.
