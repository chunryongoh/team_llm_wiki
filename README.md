...
Merge-time ingest writes generated wiki pages and `automation/.cache/compiled/<packet-id>.json`; each rendered packet page links to its compiled packet JSON. Direct bot commits use `[wiki-bot] ingest wiki packets`, reviewed bot PRs use `[wiki-bot][review-required] ingest wiki packets`, and the ingest workflow skips those bot-loop prefixes. When the default `GITHUB_TOKEN` authors bot commits, GitHub may suppress follow-up workflows; use a PAT or GitHub App token if follow-up workflows must run from bot output.
