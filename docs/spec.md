...
- `.github/workflows/wiki-llm-synthesis.yml` runs after `raw/results/wiki-ingest/**` changes reach `main` or by manual dispatch. It calls `run-llm-wiki-synthesis`, uses OpenAI Responses API `gpt-5.5` with high reasoning effort as the primary path, falls back to GitHub Models through `GITHUB_TOKEN` for recoverable provider failures, and creates a review-required bot PR containing only approved `wiki/` page replacements and an LLM synthesis report.
...
- When the OpenAI primary path is available, synthesis first runs `entity-graph`, `evidence-claims`, and `wiki-routing` specialist lanes with `gpt-5.5`, then passes their structured reports into the final `gpt-5.5` integration call.
