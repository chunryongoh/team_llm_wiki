...
The skill itself must be standalone. `team_llm_wiki` is the target repo the skill reads from and writes packets into, not the location where the skill package is implemented. The skill's source of truth should be `chunryongoh/team-llm-wiki-packet-skill`; local skill directories are installs.
