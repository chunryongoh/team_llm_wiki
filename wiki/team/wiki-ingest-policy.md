...
LLM-assisted synthesis is review-required and should run automatically after deterministic ingest output reaches `main`. The default model policy is OpenAI `gpt-5.5` for high-accuracy synthesis, but the GitHub Actions path must not have OpenAI quota as a single point of failure. The actual LLM path is `run-llm-wiki-synthesis` and `.github/workflows/wiki-llm-synthesis.yml`; it reads `AGENTS.md`, `CLAUDE.md`, `wiki/latest-context.md`, raw packet files, and the current target wiki pages, then opens a separate review-required bot PR. If OpenAI is unavailable, the workflow uses GitHub Models through `GITHUB_TOKEN`. See `wiki/team/llm-synthesis-policy.md`.
...
Scheduled `wiki-health-check` runs `check-wiki-health --stale-tentative-as-warning` so stale tentative claims appear in the health report and briefs without breaking the daily schedule. Review gates for ingest and LLM synthesis do not use that flag; they keep untracked stale tentative claims as errors. Stale claims explicitly listed in [Stale Tentative Claims](../claims/stale-tentative-claims.md) are treated as warning-level review debt until they are closed as `supported`, `disputed`, or `superseded`.
